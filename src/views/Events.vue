      ...
      <ion-list>
        <ion-card v-for="event in events" v-bind:key="event">
          <ion-card-header>
            ...
            <ion-button class="register-button" shape="round" fill="outline" v-if="eventTickets.includes(event.data.gameCode) == false && event.data.user_attendance.includes(confirmId) == false" @click="register(event.data.gameCode, event.data.date)">Register?</ion-button>
            <ion-button class="register-button" shape="round" fill="outline" v-if="eventTickets.includes(event.data.gameCode) == true && event.data.user_attendance.includes(confirmId) == false" @click="unregister(event.data.gameCode)" color="success">Unregister?</ion-button>
          </ion-card-header>
          <ion-card-content>
            <div id="basic-info">
...
export default class Events extends Vue {
    ...
    sumbit_code(e: Event) {
      ...
      for (var i=0; i<this.events.length;i++) {
        ...
        if (this.inputCode == event.data.gameCode && this.eventTickets.includes(this.inputCode) == true && event.data.user_attendance.includes(userId) == false) {
          ...
          for (var i=0;i<this.userDataList.length;i++){
            ...
            if (userData.id != userId) {
              userData.data.transactions.unshift({date: this.todayDate,
                amount: Math.round(-1*event.data.coin_return/this.userDataList.length),
          ...
          var realreturn = Math.round(Number(event.data.coin_return)/this.userDataList.length)*this.userDataList.length;
